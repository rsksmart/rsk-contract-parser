...
var _ERC = _interopRequireDefault(require("./jsonAbis/ERC165.json"));
...
class ContractParser {
  ...
  async getInterfacesByERC165Calls(address, blockNumber = 'latest', detectedInterfaces = []) {
    const candidates = Object.entries(_interfacesIds.default).
    filter(([name, { erc165 }]) => erc165 && !detectedInterfaces.includes(_types.contractsInterfaces[name] || name));
    if (!candidates.length) return [];

    const contract = new _Contract.default(_ERC.default, { address, nod3: this.nod3 });
    ...
    for (const [name, { id }] of candidates) {
      if (await supports(id)) confirmed.push(_types.contractsInterfaces[name] || name);
    }
    return confirmed;
  }
    ...
    try {
      ...
      if (ERC1167ProxyDetails.isProxy) {
        ...
        if ((0, _addresses2.isAddress)(contractDetails.implementationAddress)) {
          ...
          contractDetails.interfaces = [_types.contractsInterfaces.ERC1167, ...interfaces];
        }
      ...
      if (ERC1822ProxyDetails.isProxy) {
        ...
        if ((0, _addresses2.isAddress)(contractDetails.implementationAddress)) {
          ...
          const { methods, interfaces } = await this.getContractMethodsAndERCInterfaces(contractDetails.implementationAddress, blockNumber);
          contractDetails.methods = methods;
          contractDetails.interfaces = [_types.contractsInterfaces.ERC1822, ...interfaces];
        }
      ...
      if (ERC1967ProxyDetails.isProxy) {
        ...
        if ((0, _addresses2.isAddress)(contractDetails.implementationAddress)) {
          ...
          const { methods, interfaces } = await this.getContractMethodsAndERCInterfaces(contractDetails.implementationAddress, blockNumber);
          contractDetails.methods = methods;
          contractDetails.interfaces = [_types.contractsInterfaces.ERC1967, ...interfaces];
        }
  ...
  async isERC1167Proxy(contractAddress, blockNumber = 'latest', contractByteCode = null) {
    ...
    if (!code) {
      try {
        ...
        this.log.warn(`[${contractAddress}] Error checking bytecode for ${_types.PROXY_TYPES.ERC1167}: ${err}`);
    ...
    if (match) {
      result.isProxy = true;
      result.proxyType = _types.PROXY_TYPES.ERC1167;
