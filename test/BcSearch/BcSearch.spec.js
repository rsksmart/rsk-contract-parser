...
describe('# BC search', function () {
  ...
  describe('deploymentTx()', function () {
    this.timeout(90000)
    const tests = [
      ['0x4f82e59517c29ed0c73f5351847eb075bf473465', { blockNumber: 657077, hash: '0xb07be69b3bd8c1713d2f1772a53d2b29c17d5485bfa1f899f8b81227191a5a7a' }],
    ...
    for (const [address, expected] of tests) {
      it(`should return the deployment tx for ${address} `, async () => {
        const { tx } = await search.deploymentTx(address)
        for (const [property, value] of Object.entries(expected)) {
          console.log({ property, value, tx })
          expect(tx[property]).to.be.deep.equal(value)
        }
      })
    }
  })
